# Hosted Fields — client JS + Thymeleaf (Braintree Direct)
...
## Thymeleaf template (`templates/checkout-hosted.html`)
...
## Controller — same pattern as Drop-in

```java
@GetMapping("/checkout/hosted")
public String hosted(Model model) {
  model.addAttribute("clientToken", gateway.clientToken().generate());
  return "checkout-hosted";
}
```
